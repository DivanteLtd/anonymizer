...
RSpec.describe '#anonymize magento 1.9 sample' do
  context 'all' do
    before(:context) do
      @project_name = 'magento_1_9_scenerios_sample'
      @project_name_v1 = 'magento_1_9_sample'
      ...
      open('/tmp/' + @project_name + '.sql.gz', 'wb') do |f|
        f << open('https://github.com/DivanteLtd/anonymizer/files/2135881/' + @project_name_v1 + '.sql.gz').read
      end
    ...
    it 'should anonymize magento 1.9 scenerio sample' do
